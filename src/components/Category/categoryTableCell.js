...
class CategoryTableCell extends React.Component {
  ...
  onDetails = async () => {
    ...
    await handleOpen(category);
  };
  onDisable = () => {
    const { onCategoryActions, data } = this.props;
    const { disableCategoryRequest, getListCategory } = onCategoryActions;
    Swal.fire({
      title: data.isDisable === 1 ? "Ngưng hoạt động" : "Kích hoạt",
      text:
        data.isDisable === 1
          ? "Bạn muốn ngưng hoạt động danh mục này ?"
          : "Bạn muốn kích hoạt danh mục này ?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        await disableCategoryRequest(data.categoryID);
  ...
  render() {
    ...
    const { categoryName, isDisable } = this.props.data;
...
export default connect(mapStateToProps, mapDispatchToProps)(CategoryTableCell);
